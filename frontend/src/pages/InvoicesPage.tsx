        ...
        filteredInvoices.map((inv) => (
         <tr key={inv.id} className="hover:bg-muted transition-colors group">
          ...
          <td className="px-6 py-4">
           ...
           {inv.status === 2 && inv.paymentReference && (
            <div className="text-xs text-muted-foreground mt-0.5">Ref: {inv.paymentReference}</div>
           )}
          </td>
          <td className="px-6 py-4">
          ...
          <td className="px-6 py-4 text-right">
           <div className="flex justify-end space-x-2">
            ...
            {/* If Issued, allow Admin to Mark As Paid */}
            {inv.status === 1 && (
             <button
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
     ...
     <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6">
      <div className="flex justify-between items-center mb-4">
       ...
       <button onClick={() => setPaymentModal({ isOpen: false, invoiceId: null })} className="text-muted-foreground hover:text-foreground">
        <X className="h-5 w-5" />
       </button>
      </div>
      <p className="text-sm text-muted-foreground mb-4">
       Enter the check number, credit card authorization code, or any other payment reference. This is <strong>optional</strong> but helps track payments.
      </p>
      <div className="mb-4">
       <label className="text-xs font-semibold text-muted-foreground mb-1 block">Payment Reference (Check #, CC Auth, etc.)</label>
       <input
        type="text"
        value={paymentRef}
        onChange={e => setPaymentRef(e.target.value)}
        onKeyDown={e => e.key === 'Enter' && handleConfirmPaid()}
        placeholder="e.g., Check #4521 or CC Auth: 89234"
        autoFocus
      ...
      <div className="flex gap-3">
       <button
        onClick={() => setPaymentModal({ isOpen: false, invoiceId: null })}
        className="flex-1 border border-border text-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-muted transition-colors"
       >
        Cancel
       </button>
       <button
